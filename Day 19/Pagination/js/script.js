...
function createButtons() {
    ...
    btncontainer.innerHTML = `<button class="btn active" value= "1" >First</button ><button class="btn" value="${currentBtn}" id="101">Previous</button>`;

    for (let i = 2; i <= 10; i++) {

        btncontainer.innerHTML += `<button class="btn" value="${i}">${i}</button>`;

    }



    containerdiv.append(btncontainer);
...
tablediv.setAttribute("class", "tabled");
...
button.forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        if (btn.id != "101") {
            ...
            clearActives();
            currentBtn = btn.value;
            createRows(currentBtn);
            ...
            if (currentBtn == 1) {
                ...
                createRows(currentBtn);
                button[currentBtn].classList.add("active");
